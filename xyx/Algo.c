...
void *alloc(void *a){
	...
	fp = fopen("/media/shrey/Data/SEM4/OS/Output.osproject","a");
	int p=(int)a;
		fun1(p);
		...
		pthread_mutex_lock(&available);
		fprintf(fp,"Resource Allocated to process %d\n",p+1);
		for(int i=0;i<resources;i++){
			fprintf(fp,"now Available resource %d is :%d\n",i+1,avail[i]);
		}
		for(int i=0;i<resources;i++){
			fprintf(fp,"Total Allocated resource %d is :%d\n",i+1,allocated[p][i]);
		}
		...
		fprintf(fp,"Process %d complete, Resources given back to available\n",p+1);
		for(int i=0;i<resources;i++){
			fprintf(fp,"Now Available resource %d is :%d\n",i+1,avail[i]);
		}
		done[p]=1;
